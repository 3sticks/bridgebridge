...
var dateyboy = ""
class CalandarViewController: DayViewController {
  ...
  override func viewDidLoad() {
    ...
    //going to use this code to be able to put "today" and stuff
//    let date = Date()
//    let dateFormatter = DateFormatter()
//    dateFormatter.dateFormat = "MM/dd/YY"
//    let tmoney = dateFormatter.string(from: date)
//
//
//
    //using global variable dateyboy which is set in the eventsForDate function
    title = dateyboy
    //true makes them a weird gray
    navigationController?.navigationBar.isTranslucent = false
    self.tabBarController?.tabBar.isTranslucent = false
    dayView.autoScrollToFirstEvent = true
    
    //hugo comment
    //since you are setting the title of the navbar up there^ you need to set the tabbar title here or else it will change when you cilck on it
    self.tabBarController?.tabBar.items?[2].title = "Calendar"
    reloadData()
  }
  ...
  override func eventsForDate(_ date: Date) -> [EventDescriptor] {
    ...
    //get the date thats being used here
    let dateFormatter = DateFormatter()
    dateFormatter.dateFormat = "MM/dd/YY"
    let tmoney = dateFormatter.string(from: date)
    dateyboy = tmoney
    //add the date to the title.... TODO is this the best place for it?
    title = dateyboy
